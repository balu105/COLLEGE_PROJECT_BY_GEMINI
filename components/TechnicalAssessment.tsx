...
import { generateTechnicalQuestions } from '../services/geminiService';
...
// Added 'role' to props to provide necessary context for generating technical questions
const TechnicalAssessment: React.FC<TechnicalAssessmentProps> = ({ role, skills, onComplete }) => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        // Fix: Pass both 'role' and 'skills' to generateTechnicalQuestions as required by the service signature
  ...
  const handleNext = () => {
    if (currentIndex < questions.length - 1) {
      ...
      // Corrected: Passing the required TechnicalScore interface properties
      onComplete({
        score,
